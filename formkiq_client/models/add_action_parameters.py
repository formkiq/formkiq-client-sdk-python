...
class AddActionParameters(BaseModel):
    ...
    ocr_engine: Optional[StrictStr] = Field(default=None, description="OCR: Engine to use for Optical character recognition", alias="ocrEngine")
    add_pdf_detected_characters_as_text: Optional[StrictBool] = Field(default=None, description="OCR: For the rewriting of the PDF document, converting any image text to searchable text", alias="addPdfDetectedCharactersAsText")
    ...
    queue_name: Optional[StrictStr] = Field(default=None, description="Name of Queue action", alias="queueName")
    __properties: ClassVar[List[str]] = ["ocrParseTypes", "ocrEngine", "addPdfDetectedCharactersAsText", "url", "characterMax", "engine", "notificationType", "notificationToCc", "notificationToBcc", "notificationSubject", "notificationText", "notificationHtml", "tags", "queueName"]

    @field_validator('ocr_engine')
    def ocr_engine_validate_enum(cls, value):
        """Validates the enum"""
        if value is None:
            return value

        if value not in ('TESSERACT', 'TEXTRACT'):
            raise ValueError("must be one of enum values ('TESSERACT', 'TEXTRACT')")
        return value
